...
const STYLES: Record<DataSource, { label: string; className: string }> = {
  api: { label: "실시간 데이터", className: "bg-primary text-on-primary" },
...
export default function DataSourceBadge({ source, fetchedAt }: { source: DataSource; fetchedAt?: string }) {
  ...
  const time =
    fetchedAt && (source === "cache" || source === "stale-cache" || source === "api")
      ? new Date(fetchedAt).toLocaleString("ko-KR", { dateStyle: "short", timeStyle: "short" })
      : null;
  return (
