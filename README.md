...
4. **Actions 탭 → "Deploy to GitHub Pages" → Run workflow** 로 첫 배포 실행
   → `https://<계정>.github.io/<저장소이름>/` (이후 main 푸시·매일 새벽 자동 배포)
...
- 데이터를 받지 못한 데이터셋은 저장소에 커밋된 `data/live/*.json` → 내장 샘플 데이터 순으로 대체됩니다 (샘플 사용 시 화면에 "샘플 데이터" 배지 표시)

### ⚠ 식약처 API의 해외 IP 차단과 데이터 갱신

식품안전나라 API는 해외 IP를 차단하는 경우가 있어, 미국에 있는 GitHub Actions 서버에서는
데이터 수신이 실패할 수 있습니다 (HTML 차단 페이지 수신). 이 경우 **한국 PC에서 받아서 커밋**하면 됩니다:

```bash
npm run update-data          # 한국 IP에서 실행 → data/live/*.json 갱신
git add data/live && git commit -m "데이터 갱신" && git push
```
